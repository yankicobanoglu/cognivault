...
const SPEED_SETTINGS: Record<string, number> = {
  slow: 3500,
  normal: 2500,
  fast: 1500,
};
...
const PHONETIC_MAP_TR: Record<string, string> = {
  'A': 'aa',
  'E': 'eee',
  'İ': 'iii',
  'O': 'oo',
...
const speakLetter = (letter: string, lang: Language, voice?: SpeechSynthesisVoice | null) => {
  const synth = window.speechSynthesis;
  resumeAudio();
  synth.cancel();
...
const loadVoices = (lang: Language, callback: (voice: SpeechSynthesisVoice | null) => void) => {
  const synth = window.speechSynthesis;
  let attempts = 0;
...
const generateSequence = (level: number, mode: GameMode, length: number, letters: string[], seed?: number): Stimulus[] => {
